...
def discretized_mix_logistic_loss_map(x, l):
    """ log-likelihood for mixture of discretized logistics, assumes the data
    has either 3 or 2 channels, and has been rescaled to [-1,1] interval """
    # Expects x to have shape [B, C, H, W] (pytorch ordering); we reshape to
    # tensorflow ordering (channel-last) below.
...
def uint16_to_uint8(img):
    return img.view(torch.uint8)[..., :2]

class H5Dataset(Dataset):
    def __init__(self, h5dir, transform=None):
        with h5py.File(h5dir, 'r') as f:
            self.data = f['images'][:]
        print(self.data.shape, 'data input')
    ...
    def __getitem__(self, idx):
        image = uint16_to_uint8(torch.tensor(self.data[idx])).cpu().numpy()
...
class YiboDataset(Dataset):
    def __init__(self, transform=None):
        self.data = np.random.randint(0, 2**16, (100_000, 32,32, 1))
        self.transform = transform
