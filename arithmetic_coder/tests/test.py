...
def test_uniform_float():
  ...
  cdf_int = torchac._convert_to_int_and_normalize(cdf_float,
                                                  needs_normalization=False)
  assert cdf_int[0, 1] == int(torch.tensor(1/3) * 2 ** 31)
  assert cdf_int[0, -1] == torch.tensor(2 ** 31, dtype=torch.uint32).to(torch.int32)
...
def test_uniform_float_multipdim():
  ...
  sym = torch.arange(C * H * W, dtype=torch.int32) % L
...
def _encode_decode(cdf_float, symbols_to_check,
  ...
  for symbol in symbols_to_check:
    sym = torch.tensor([symbol], dtype=torch.int32)
    byte_stream = torchac.encode_float_cdf(
      cdf_float,
      sym,
