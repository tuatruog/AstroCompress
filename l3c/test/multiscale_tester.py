...
from utils.data_loader import L3CCompressHfDataset, L3CCompressLocalDataset, get_dataset_hf, get_dataset_local
from utils.transform_utils import uint16_to_uint8
from utils.img_utils import crop_img, pad_img, unpad_img
from l3c.bitcoding import part_suffix_helper
from l3c.blueprints.multiscale_blueprint import MultiscaleBlueprint, MultiscaleLoss
from l3c.dataloaders.images_loader import IndexImagesDataset
from l3c.helpers import paths, saver, logdir_helpers
from l3c.helpers.config_checker import DEFAULT_CONFIG_DIR
from l3c.helpers.global_config import global_config
from collections import namedtuple

from l3c.test import cuda_timer
from l3c.test.image_saver import ImageSaver
...
class MultiscaleTester(object):
    def __init__(self, log_date, flags, restore_itr, dataset_type, l3c=False):
        """
        :param flags:
            log_dir
            img
        ...
        self.ms_config_p = flags.ms_config_p
        test_log_dir_root = self.flags.log_dir.rstrip(os.path.sep) + '_test'
        global_config.reset()

        config_ps, experiment_dir = self.get_configs_experiment_dir('ms', self.flags.log_dir, log_date)
        ...
        if self.flags.write_to_files:
            ...
            from l3c.bitcoding.bitcoding import Bitcoding
            self.bc = Bitcoding(self.flags, self.blueprint, times=self.times, compare_with_theory=self.flags.compare_theory)
        elif l3c:  # Called from l3c.py
            from l3c.bitcoding.bitcoding import Bitcoding
            self.bc = Bitcoding(self.flags, self.blueprint, times=no_op.NoOp)
    ...
    def get_configs_experiment_dir(self, prefix, log_dir, log_date):
        experiment_dir = paths.get_experiment_dir(log_dir, log_date)
        log_dir_comps = logdir_helpers.parse_log_dir(
                experiment_dir, self.ms_config_p, [prefix, 'dl'], append_ext='.cf')
    ...
    def get_test_dataset(self, testset):
        if self.dataset_type == 'astro':
            transform = None
            if self.flags.split_bits:
                transform = uint16_to_uint8

            # Remote huggingface dataset
            # (_, _, _ds_test), extract_fn = get_dataset_hf(self.flags.dataset)
            # return L3CCompressHfDataset(_ds_test, extract_fn, transform)

            # Local huggingface dataset
            (_, _, _ds_test), root, ext_fn = get_dataset_local(self.flags.dataset)
            return L3CCompressLocalDataset(root, _ds_test, ext_fn, transform)
        else:
            to_tensor_transform = [IndexImagesDataset.to_tensor_uint8_transform()]
            if self.flags.crop_img:
                print('*** WARN: Cropping to {}'.format(self.flags.crop_img))
                to_tensor_transform.insert(0, transforms.CenterCrop(self.flags.crop_img))
            return IndexImagesDataset(
                    testset,
                    to_tensor_transform=transforms.Compose(to_tensor_transform))

    def _test(self, ds):
        h, w = self.flags.model_input_size
        ...
        for i, data in enumerate(ds):
            if self.dataset_type == 'astro':
                filename = data['name'] + f".{data['idx']}"
            else:
                filename = os.path.splitext(os.path.basename(ds.files[data['idx']]))[0]
            ...
            if self.flags.write_to_files:
                print('***', filename)
                img_raw = data["raw"].long().to(pe.DEVICE)  # 1CHW
            ...
            raw_img = data["raw"]  # Full resolution image, !CHW

            # Make sure we count bpsp of different crops of an image correctly.
            combinator = l3c.auto_crop.CropLossCombinator()

            raw_img, padding_tuple = pad_img(raw_img, patch_size=(h, w))
            raw_img_crops = crop_img(raw_img, h, w, batchify=True)

            starter.record()
            for raw_img_crop in raw_img_crops:
                ...
                num_subpixels_before_pad = np.prod(raw_img_crop.shape)
                img_batch, _ = self.blueprint.unpack_batch_pad(raw_img_crop, fac=self._padding_fac())
            ...
            number_of_crops[raw_img_crops.shape[0]] += 1
