...
import l3c.bitcoding.coders_helpers
import l3c.pytorch_ext as pe
from l3c.bitcoding.coders import ArithmeticCoder
from l3c.bitcoding import part_suffix_helper
from l3c.blueprints.multiscale_blueprint import MultiscaleBlueprint
from l3c.test import cuda_timer
from l3c.helpers import pad
...
class Bitcoding(object):
    ...
    """
    def __init__(self, flags, blueprint, times: cuda_timer.StackTimeLogger, compare_with_theory=False):
        self.flags = flags
        self.blueprint = blueprint
        self.compare_with_theory = compare_with_theory
        self.times = times

    def encode(self, raw_img, pout):
        """
        Encode image to disk at path `p`.
        :param raw_img: tensor of shape CHW or 1CHW
        :param pout: path
        :return actual_bpsp
        """
        assert not os.path.isfile(pout)
        assert len(raw_img.shape) == 3, raw_img.shape
        assert raw_img.dtype == torch.int64, raw_img.dtype

        c = l3c.auto_crop.CropLossCombinator()

        _, h, w = self.flags.model_input_size

        raw_img, padding_tuple = pad_img(raw_img, patch_size=(h, w))
        raw_img_crops = crop_img(raw_img, h, w, batchify=False)
        # Extract X,Y patch location
        X, Y, C, H, W = raw_img_crops.shape
        raw_img_crops = raw_img_crops.reshape(X * Y, C, H, W)
        for i, img_crop in enumerate(raw_img_crops):
            bpsp_crop = self._encode(img_crop.unsqueeze(0), pout + part_suffix_helper.make_part_suffix(i),
                                     part_idx=i, full_padding_tuple=padding_tuple, XY=(X, Y))
            c.add(bpsp_crop, np.prod(img_crop.shape[-2:]))
        return c.get_bpsp()

    def _encode(self, img, pout, part_idx, full_padding_tuple, XY):
        ...
        with open(pout, 'wb') as fout:
            # write the full padding tuple and X Y in the first part
            if part_idx == 0:
                write_padding_tuple(full_padding_tuple, fout)
                write_bytes(fout, [np.uint16, np.uint16], [XY[0], XY[1]])
    ...
    def decode(self, pin, _recurse_part=True, first_part=False):
        """
        :param pin:  Path where image is stored
        :param _recurse_part: If True and `pin` ends in .partX, iterate over all parts and stitch!
        :return: Decoded image, as 1CHW, long
        """
        if _recurse_part and part_suffix_helper.contains_part_suffix(pin):
            parts = []
            pin_parts = part_suffix_helper.iter_part_suffixes(pin)
            out, full_pad, X, Y = self.decode(pin_parts[0], _recurse_part=False, first_part=True)
            parts.append(out)
            for pin_part in pin_parts[1:]:
                parts.append(self.decode(pin_part, _recurse_part=False, first_part=False)[0])

            print(f'Stitching {len(parts)} parts...')
            parts = torch.stack(parts, dim=0).squeeze(1)
            img = uncrop_img(parts, X, Y)
            img = unpad_img(img.squeeze(0), full_pad)
            return img

        full_padding_tuple = None
        X = None
        Y = None
        with open(pin, 'rb') as fin:
            if first_part:
                full_padding_tuple = read_padding_tuple(fin)
                X, Y = read_bytes(fin, [np.uint16, np.uint16])
                X = X[0]
                Y = Y[0]
        ...
        return out, full_padding_tuple, X, Y
    ...
    def code_with_cdf(self, l, bn_shape, bn_coder, dmll):
        ...
        """
        N, C, H, W = bn_shape
        coding = l3c.bitcoding.coders_helpers.CodingCDFNonshared(
...
def _get_cdf_from_pr(pr):
    ...
    cdf = torch.cat((torch.zeros((N, H, W, 1), dtype=cdf.dtype, device=cdf.device),
                     cdf), dim=-1)
    cdf = cdf.to('cpu', dtype=torch.uint32, non_blocking=True).int()
