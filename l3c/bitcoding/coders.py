...
from l3c.criterion.logistic_mixture import CDFOut
from l3c.test.cuda_timer import StackTimeLogger
...
class ArithmeticCoder(object):
    ...
    def range_encode(self, data, cdf, time_logger: StackTimeLogger):
        ...
        assert data.dtype == torch.int32, 'Wrong dtype: {}'.format(data.dtype)
        ...
        if isinstance(cdf, CDFOut):
            ...
            with time_logger.run('ac.encode'):
                cdf = torchac.get_int32_normalized_cdf(logit_probs_c_sm, targets, means_c, log_scales_c)
                out_bytes = torchac.encode_int32_normalized_cdf(cdf, data)
        else:
            N, H, W, Lp = cdf.shape
    ...
    def range_decode(self, encoded_bytes, cdf, time_logger: StackTimeLogger = no_op.NoOp):
        ...
        if isinstance(cdf, CDFOut):
            ...
            with time_logger.run('ac.encode'):
                cdf = torchac.get_int32_normalized_cdf(logit_probs_c_sm, targets, means_c, log_scales_c)
                decoded = torchac.decode_int32_normalized_cdf(cdf, encoded_bytes)
