...
from l3c.modules import quantizer
...
def non_shared_get_Kp(K, C):
    ...
    if C == 3:  # finest scale
        ...
        return _NUM_PARAMS_OTHER * C * K + 1 * K
    else:
...
def non_shared_get_K(Kp, C):
    ...
    if C == 3:
        ...
        return Kp // (_NUM_PARAMS_OTHER * C + 1)
    else:
...
class DiscretizedMixLogisticLoss(l3c.vis.summarizable_module.SummarizableModule):
    ...
    def _extract_non_shared(self, x, l):
        ...
        if C != 2:
            ...
            l = l.reshape(N, (C * _NUM_PARAMS_OTHER + 1) * K, H, W)
            logit_probs = l[:, 0:C*K, ...].reshape(N, C, K, H, W) # NCKHW
            means = l[:, C*K:2*C*K, ...].reshape(N, C, K, H, W) # NCKHW
            log_scales = torch.clamp(l[:, 2*C*K:3*C*K, ...].reshape(N, C, K, H, W), min=_LOG_SCALES_MIN)
            x = x.reshape(N, C, 1, H, W)

            coeffs = self._nonshared_coeffs_act(l[:, 3*C*K:, ...]).reshape(N, 1, K, H, W)
            means_0, means_1 = means[:, 0, ...], means[:, 1, ...]
            coeffs_1 = coeffs[:, 0, ...] # NKHW
            means = torch.stack(
                (means_0,
                 means_1 + coeffs_1 * x[:, 0, ...]), dim=1)
    ...
    def _extract_non_shared_c(self, c, C, l, x=None):
        ...
        if C != 2:
            ...
            # Assuming C == 2 only on input layer for 8-bit 2 channel split, and other Z(s) layer have C != 2
            l = l.reshape(N, (C * _NUM_PARAMS_OTHER + 1) * K, H, W)
            logit_probs_c = l[:, 0:C * K, ...].reshape(N, C, K, H, W)[:, c, ...]  # NKHW
            means_c = l[:, C * K:2 * C * K, ...].reshape(N, C, K, H, W)[:, c, ...]  # NKHW
            log_scales_c = torch.clamp(l[:, 2 * C * K:3 * C * K, ...].reshape(N, C, K, H, W)[:, c, ...], min=_LOG_SCALES_MIN) # NKHW, is >= -7

            unscaled_coeffs = l[:, 3*C*K:, ...].reshape(N, 1, K, H, W) # NCKHW
            if c == 1:
                assert x is not None
                coeffs_1 = torch.sigmoid(unscaled_coeffs[:, 0, ...]) # NKHW
                means_c += coeffs_1 * x[:, 0, ...]
...
def _visualize_params(logits_pis, means, log_scales, channel):
    ...
    grid = l3c.vis.grid.prep_for_grid(mixtures)
