...
from l3c.criterion.logistic_mixture import DiscretizedMixLogisticLoss
from l3c.modules import edsr
from l3c.helpers.global_config import global_config
from l3c.modules.head import RGBHead, U16Head, U8Head, Head
from l3c.modules.net import Net, EncOut
from l3c.modules.prob_clf import AtrousProbabilityClassifier
...
class Losses(l3c.vis.summarizable_module.SummarizableModule):
    def __init__(self, config_ms):
        super(Losses, self).__init__()

        if config_ms.img_type == 'I;16B':
            # rgb scale is false here since we don't need lambda
            self.loss_dmol_rgb = DiscretizedMixLogisticLoss(
                rgb_scale=False, x_min=0, x_max=65535, L=65536)
        elif config_ms.img_type == 'I;8B':
            self.loss_dmol_rgb = DiscretizedMixLogisticLoss(
...
class MultiscaleNetwork(l3c.vis.summarizable_module.SummarizableModule):
    def __init__(self, config_ms):
        ...
        if config_ms.img_type == 'I;16B':
            rgb_mean = (0.5,)
            rgb_std = (1.0,)
            self.sub_rgb_mean = edsr.MeanShift(65535., rgb_mean, rgb_std, in_channels=1, out_channels=1)

            heads = ([U16Head(config_ms)] +
                     [Head(config_ms, Cin=self.get_Cin_for_scale(scale))
                      for scale in range(self.scales - 1)])
            nets = [Net(config_ms, scale)
                    for scale in range(self.scales)]
            prob_clfs = ([AtrousProbabilityClassifier(config_ms, C=1)] +
            ...
            self.sub_rgb_mean = edsr.MeanShift(255., rgb_mean, rgb_std, in_channels=2, out_channels=2)

            heads = ([U8Head(config_ms)] +
                     [Head(config_ms, Cin=self.get_Cin_for_scale(scale))
                      for scale in range(self.scales - 1)])
            nets = [Net(config_ms, scale)
                    for scale in range(self.scales)]
            prob_clfs = ([AtrousProbabilityClassifier(config_ms, C=2)] +
                         [AtrousProbabilityClassifier(config_ms, config_ms.q.C)
                          for _ in range(self.scales - 1)])
