...
import l3c.vis.summarizable_module
from l3c.helpers import logdir_helpers
import l3c.pytorch_ext as pe
import l3c.vis.safe_summary_writer
from l3c.blueprints.multiscale_blueprint import MultiscaleBlueprint
from l3c.dataloaders import images_loader
from utils.data_loader import L3CCompressHfDataset, L3CCompressLocalDataset, CustomDataLoader, get_dataset_hf, get_dataset_local
from utils.transform_utils import build_transform_fn
from l3c.helpers.global_config import global_config
from l3c.helpers.paths import CKPTS_DIR_NAME
from l3c.helpers.saver import Saver
from l3c.train import lr_schedule
from l3c.train.train_restorer import TrainRestorer
from l3c.train.trainer import LogConfig, Trainer
...
class MultiscaleTrainer(Trainer):
    def __init__(self,
                 ...
                 loader_type='astro',
                 sw_cls=l3c.vis.safe_summary_writer.SafeSummaryWriter):
        """
        ...
        :param loader_type: 'astro' for AstroCompression dataloader
        ...
        self.summarizer = l3c.vis.summarizable_module.Summarizer(sw)
    ...
    def _get_dataloaders(self, num_workers, shuffle_train=True):
        ...
        if self.loader_type == 'astro':
            transform = build_transform_fn(self.args)

            # Remote huggingface dataset
            # (_ds_train, _ds_val, _), extract_fn = get_dataset_hf(self.args.dataset)
            # ds_train = L3CCompressHfDataset(_ds_train, extract_fn, transform)
            # ds_val = L3CCompressHfDataset(_ds_val, extract_fn, transform)

            # Local huggingface dataset
            (_ds_train, _ds_val, _), root, extract_fn = get_dataset_local(self.args.dataset)
            ds_train = L3CCompressLocalDataset(root, _ds_train, extract_fn, transform)
            ds_val = L3CCompressLocalDataset(root, _ds_val, extract_fn, transform)

            dl_train = CustomDataLoader(ds_train, self.args.batch_size, shuffle=True, max_batch=self.args.n_batch_train)
            dl_val = CustomDataLoader(ds_val, self.args.batch_size, shuffle=True,  max_batch=self.args.n_batch_val)
        else:
