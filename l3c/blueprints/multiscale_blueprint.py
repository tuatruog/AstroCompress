...
from l3c.helpers.pad import pad
from l3c.modules.multiscale_network import MultiscaleNetwork, Out
from l3c.vis import histogram_plotter, image_summaries
...
class MultiscaleBlueprint(l3c.vis.summarizable_module.SummarizableModule):
...
def new_bottleneck_summary(s, L):
    ...
    _assert_contains_symbol_indices(s, L)
    s = s.float().div(L)
    grid = l3c.vis.grid.prep_for_grid(s, channelwise=True)
